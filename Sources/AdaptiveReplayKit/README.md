...
- **Trace recording** (`Sources/CodexBar/AdaptiveRefreshTraceRecording.swift`, app target):
  ...
  live schedule comparison; `timerAdvanced` remains the accepted subset.
...
`AdaptiveRefreshPolicy`. The fork-only `CodingActivityAdaptivePolicy` uses the A-layer signal in
